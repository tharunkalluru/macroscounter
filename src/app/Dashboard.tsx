  ...
  return (
    <div className="mx-auto max-w-5xl px-5 pb-4 lg:px-8" data-testid="today-view">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight sm:text-display">{isToday ? 'Today' : 'Your diary'}</h1>
        </div>
      ...
      <section className="mb-5 grid gap-2 sm:grid-cols-[1fr_1fr] sm:gap-3" aria-label="Logging shortcuts">
        <Link to={`/log/add?date=${date}`} className="pressable flex min-h-14 items-center justify-between gap-3 rounded-2xl bg-brand-700 px-5 py-3 text-sm font-semibold text-white">
          <span>Log food</span><PlusIcon />
        </Link>
        <div className="grid grid-cols-4 gap-2">
          <Link to={`/scan?date=${date}`} aria-label="Scan barcode" className="quick-action pressable"><BarcodeIcon /><span>Scan</span></Link>
          <Link to={`/log/ai?date=${date}`} aria-label="AI meal: describe or photograph your food" className="quick-action pressable"><SparkleIcon /><span>AI meal</span></Link>
          <Link to={`/log/quick-add?date=${date}`} className="quick-action pressable"><ForkKnifeIcon /><span>Quick add</span></Link>
          <Link to={`/templates?date=${date}`} aria-label="Saved meals" className="quick-action pressable"><LogIcon /><span>Saved</span></Link>
        </div>
      </section>
      <div className="grid items-start gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.05fr)]">
        <div className="min-w-0 space-y-5">
          ...
          >
            <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
              ...
              {isToday && dayOfProgram !== null && <span className="rounded-full bg-slate-50 px-2.5 py-1 text-caption text-brand-700 dark:bg-slate-800 dark:text-brand-400" data-testid="today-program-header">Day {dayOfProgram}</span>}
            </div>
            <CaloriesRing consumedKcal={totals.kcal} targetKcal={target.kcal} />
            {!targets && <p className="mt-1 text-center text-caption text-slate-500 dark:text-slate-400">No target for this day</p>}
        ...
        <div className="min-w-0 space-y-5">
          ...
          <TodayEntryList entries={entries} date={date} isToday={isToday} onDelete={handleDelete} />
          <WeeklyDiaryCard date={date} entries={historyEntries} onSelect={goToDate} />
          {isToday && <DailyBrief entries={entries} />}
        </div>
      </div>
