...
export function CoachMessage({ children, testId }: { children: ReactNode; testId?: string }) {
  return (
    <div className="flex items-start gap-2.5">
      ...
      <div
        data-testid={testId}
        className="min-w-0 max-w-[85%] rounded-2xl rounded-bl-md bg-slate-100 px-4 py-3 text-sm leading-relaxed text-slate-900 dark:bg-surface-dark-card dark:text-slate-100"
      >
        {children}
      </div>
    </div>
