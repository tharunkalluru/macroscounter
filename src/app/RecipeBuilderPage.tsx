...
export default function RecipeBuilderPage() {
  ...
  const preview = useMemo(() => {
    ...
    const input: RecipeIngredientInput[] = ingredients.map((i) => ({ foodId: i.food.id, grams: i.grams }))
    try {
  ...
  async function handleSave() {
    ...
    if (!Number.isFinite(servingsNum) || servingsNum <= 0) return setError('Servings must be at least 1.')
  ...
  return (
    <div className="mx-auto max-w-md px-6 py-8">
      ...
      <div className="mt-4">
        ...
        />
        {query.trim() && (
          <ul className="mt-1 divide-y divide-slate-100 rounded-lg bg-white shadow-sm">
            {results.map((food) => (
              <li key={food.id}>
                <button
                  type="button"
      ...
      {preview && (
        <p className="mt-4 text-sm text-slate-600" data-testid="recipe-preview">
          Per serving ({preview.gramsPerServing} g): {Math.round((preview.computedPer100g.kcal * preview.gramsPerServing) / 100)}{' '}
          kcal
        </p>
      )}
