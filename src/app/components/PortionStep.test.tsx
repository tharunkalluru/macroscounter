...
const per100g = { kcal: 150, p: 12, c: 18, f: 6 }
const referencePortions: Portion[] = [{ label: '1 cup', grams: 80 }]

describe('PortionStep', () => {
  it('defaults the grams field to the first reference portion', () => {
    render(<PortionStep per100g={per100g} referencePortions={referencePortions} onSave={vi.fn()} />)
  ...
  it('a reference-portion chip fills the field with its own gram value, not a raw multiplier', () => {
    render(<PortionStep per100g={per100g} referencePortions={referencePortions} onSave={vi.fn()} />)
  ...
  it('the log button shows the live "Add {grams} g · {kcal} kcal" result and saves grams-only', () => {
    const onSave = vi.fn()
    render(<PortionStep per100g={per100g} referencePortions={referencePortions} onSave={onSave} />)
  ...
  it('edit mode pre-fills from initialGrams, not the first reference portion', () => {
    render(
      <PortionStep
        per100g={per100g}
  ...
  it('quickGrams=[] omits the fixed chips, showing only the reference portions (scanned-product usage)', () => {
    render(
      <PortionStep
        per100g={per100g}
        referencePortions={[{ label: '1 pack', grams: 75 }]}
        quickGrams={[]}
        onSave={vi.fn()}
      />
    )
    expect(screen.queryByTestId('gram-chip-100')).not.toBeInTheDocument()
    expect(screen.getByTestId('gram-chip-portion')).toHaveTextContent('1 pack ≈ 75 g')
  })
})
